...
{
    if(arvore == NULL || raiz == NULL) return false;
    if(arvore->item.chave == raiz->item.chave && arvore->pai != raiz->pai && arvore->esq != raiz->esq && arvore->dir != raiz->dir){
        return true;
    }
    else{
        if(arvore == NULL || raiz == NULL){
            return false;
        }
        else{
            return haDuplicados(arvore->esq, raiz) || haDuplicados(arvore->dir, raiz) || haDuplicados(arvore, raiz->esq) || haDuplicados(arvore, raiz->dir);
        }
    }
}
